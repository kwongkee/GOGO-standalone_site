...
(function($) {
	...
	$.baseurl = function(path, tag){
		...
		if (path && tag && path.indexOf("/" + tag) == -1 && path.indexOf("/") == 0) {
			return "/" + tag + path;
		}
	...
	$.initTag = function() {
		if (szy_tag) {
			$("a").not("[szy_tag_compiled]").each(function() {
				var href = $(this).attr("href");

				if (href && href.indexOf("/" + szy_tag) == -1 && href.indexOf("/") == 0) {
					$(this).attr("href", "/" + szy_tag + href);
				}
			...
			$("form").not("[szy_tag_compiled]").each(function() {
				...
				if (action && action.indexOf("/" + szy_tag) == -1 && action.indexOf("/") == 0) {
					$(this).attr("action", "/" + szy_tag + action);
				}
