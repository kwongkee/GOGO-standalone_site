...
translate.request.api.host+translate.request.api.language
 */
var translate = {
    ...
    selectLanguageTag: {
        show: !0, languages: "", alreadyRender: !1, selectOnChange: function (e) {
            ...
            setTimeout(function() {
                if (!translate.selectLanguageTag.alreadyRender && (translate.selectLanguageTag.alreadyRender = !0, translate.selectLanguageTag.show)) {
                    if (null == document.getElementById("translate")) {
                        var e = document.getElementsByTagName("body")[0], t = document.createElement("div");
                        t.id = "translate", e.appendChild(t)
                    } else if (null != document.getElementById("translateSelectLanguage")) return;
                    
                    // 获取 CSRF 令牌
                    var csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
                    translate.request.post("https://backend.gogo198.cn/getLanguage", {'_token':csrfToken}, function (e) {
                        if (0 != e.result) {
                            var t = function (e) {
                            ...
                            for (var n = 0; n < e.list.length; n++) {
                                ...
                                if (l.setAttribute("value", e.list[n].id), translate.selectLanguageTag.languages.length > 0) {
                                    ...
                                    if (r.indexOf("," + e.list[n].id.toLowerCase() + ",") < 0) continue
                                }
    ...
    changeLanguage: function (e) {
        if (",en,de,hi,lt,hr,lv,ht,hu,zh-CN,hy,uk,mg,id,ur,mk,ml,mn,af,mr,uz,ms,el,mt,is,it,my,es,et,eu,ar,pt-PT,ja,ne,az,fa,ro,nl,en-GB,no,be,fi,ru,bg,fr,bs,sd,se,si,sk,sl,ga,sn,so,gd,ca,sq,sr,kk,st,km,kn,sv,ko,sw,gl,zh-TW,pt-BR,co,ta,gu,ky,cs,pa,te,tg,th,la,cy,pl,da,tr,".indexOf("," + e + ",") > -1) {
            translate.check();
            var t = "/" + translate.localLanguage + "/" + e, a = document.location.host.split(".");
            if (a.length > 2) {
        ...
        isIgnore: function (e) {
            ...
            for (var t = e, a = 100; a-- > 0;) {
                ...
                if (null != t.className) {
                    ...
                    l = l.trim().split(" ");
                    for (var r = 0; r < l.length; r++) if (null != l[r] && l[r].trim().length > 0 && translate.ignore.class.indexOf(l[r]) > -1) return !0
                }
    ...
    nomenclature: {
        data: new Array, old_Data: [], set: function (e) {
            ...
            for (var t in translate.nomenclature.data[translate.language.getLocal()][translate.to]) {
                ...
                if ("function" != typeof a) {
                    var n = e.indexOf(t);
                    if (n > -1) if ("english" == translate.language.getLocal()) {
                        var l = "";
                        if (0 == n) ; else if (l = e.substr(n - 1, 1), "english" == translate.language.getCharLanguage(l)) continue;
                        var r = "";
                        if (n + t.length == e.length) ; else if (r = e.substr(n + t.length, 1), "english" == translate.language.getCharLanguage(r)) continue;
    ...
    listener: {
        isExecuteFinish: !1, isStart: !1, start: function () {
            ...
            const e = {attributes: !0, childList: !0, subtree: !0}, t = new MutationObserver(function (e, t) {
                var a = [];
    ...
    renderTask: class {
        ...
        add(e, t, a, n) {
            var l = translate.element.nodeAnalyse.get(e, n), r = translate.util.hash(l.text);
            void 0 === this.nodes[r] && (this.nodes[r] = new Array), this.nodes[r].push(e);
            var s = this.taskQueue[r];
            null != s && void 0 !== s || (s = new Array);
            var o = new Array;
            o.originalText = t, o.resultText = a, o.attribute = n, s.push(o), this.taskQueue[r] = s
        }

        execute() {
            for (var e in this.taskQueue) {
                "function" != typeof (t = this.taskQueue[e]) && (t.sort((e, t) => t.originalText.length - e.originalText.length), this.taskQueue[e] = t)
            }
            for (var e in this.nodes) for (var t = this.taskQueue[e], a = 0; a < this.nodes[e].length; a++) for (var n = 0; n < t.length; n++) {
                var l = t[n];
            ...
            if (null != translate.to && void 0 !== translate.to && 0 != translate.to.length) {
                if (translate.to != translate.language.getLocal()) {
                    ...
                    for (var u in translate.nodeQueue[t].list) {
                        ...
                        for (var i in translate.nodeQueue[t].list[u]) if ("function" != typeof translate.nodeQueue[t].list[u][i]) {
                            ...
                            if (null != f && f.length > 0) for (var h = 0; h < translate.nodeQueue[t].list[u][i].nodes.length; h++) e.add(translate.nodeQueue[t].list[u][i].nodes[h].node, g, translate.nodeQueue[t].list[u][i].beforeText + f + translate.nodeQueue[t].list[u][i].afterText, translate.nodeQueue[t].list[u][i].nodes[h].attribute); else s[u].push(d), o[u].push(i)
                        }
                        e.execute()
                    }
                    var p = [];
                    for (var u in translate.nodeQueue[t].list) s[u].length < 1 || p.push(u);
                    if (translate.listener.isExecuteFinish || (translate.temp_executeFinishNumber = 0, translate.temp_executeFinishInterval = setInterval(function () {
                        translate.temp_executeFinishNumber == p.length && (translate.listener.isExecuteFinish = !0, clearInterval(translate.temp_executeFinishInterval))
                    }, 50)), 0 != p.length) for (var v in p) {
                        u = p[v];
                        if (void 0 === s[u] || s[u].length < 1) return;
                        var m = translate.request.api.host + translate.request.api.translate + "?v=" + translate.version,
                            x = {from: u, to: translate.to, text: encodeURIComponent(JSON.stringify(s[u]))};
                        translate.request.post(m, x, function (e) {
                            if (0 == e.result) return console.log("=======ERROR START======="), console.log("response : " + e.info), console.log("=======ERROR END  ======="), void translate.temp_executeFinishNumber++;
                            let a = new translate.renderTask;
                            for (var n = 0; n < o[e.from].length; n++) {
                                var l = e.from, r = e.text[n], u = o[e.from][n],
                                    i = translate.nodeQueue[t].list[l][u].cacheHash, g = "";
                                try {
                                ...
                                for (var d = 0; d < translate.nodeQueue[t].list[l][u].nodes.length; d++) a.add(translate.nodeQueue[t].list[l][u].nodes[d].node, g, translate.nodeQueue[t].list[l][u].beforeText + r + translate.nodeQueue[t].list[l][u].afterText, translate.nodeQueue[t].list[l][u].nodes[d].attribute);
    ...
    element: {
        nodeAnalyse: {
            get: function (e, t) {
                ...
                if (null != n && "string" == typeof n && n.length > 0) return l.text = e[n], void 0 !== t && t.length > 0 && (void 0 !== e[n] ? e[n] = e[n].replace(new RegExp(translate.util.regExp.pattern(t), "g"), translate.util.regExp.resultText(a)) : console.log(e)), l;
                "#text" == r && (void 0 !== e.parentNode && "TEXTAREA" == translate.element.getNodeName(e.parentNode) && (r = "TEXTAREA", e = e.parentNode));
                if ("INPUT" == r || "TEXTAREA" == r) {
                    if (null == e.attributes || void 0 === e.attributes) return l.text = "", l;
                    if ("INPUT" == r && void 0 !== e.attributes.type && null != typeof e.attributes.type.nodeValue && ("button" == e.attributes.type.nodeValue.toLowerCase() || "submit" == e.attributes.type.nodeValue.toLowerCase())) {
                        ...
                        if (null != s && void 0 !== s && void 0 !== s.nodeValue && s.nodeValue.length > 0) return void 0 !== t && t.length > 0 && (s.nodeValue = s.nodeValue.replace(new RegExp(translate.util.regExp.pattern(t), "g"), translate.util.regExp.resultText(a))), l.text = s.nodeValue, l.node = s, l
                    }
                    return void 0 !== e.attributes.placeholder ? (void 0 !== t && t.length > 0 && (e.attributes.placeholder.nodeValue = e.attributes.placeholder.nodeValue.replace(new RegExp(translate.util.regExp.pattern(t), "g"), translate.util.regExp.resultText(a))), l.text = e.attributes.placeholder.nodeValue, l.node = e.attributes.placeholder, l) : (l.text = "", l)
                }
                if ("META" == r) {
                    if (void 0 !== e.name && null != e.name) {
                        var o = e.name.toLowerCase();
                        if ("keywords" == o || "description" == o) return void 0 !== t && t.length > 0 && (e.content = e.content.replace(new RegExp(translate.util.regExp.pattern(t), "g"), translate.util.regExp.resultText(a))), l.text = e.content, l
                    }
                    return l.text = "", l
                }
                return "IMG" == r ? void 0 === e.alt || null == e.alt ? (l.text = "", l) : (void 0 !== t && t.length > 0 && (e.alt = e.alt.replace(new RegExp(translate.util.regExp.pattern(t), "g"), translate.util.regExp.resultText(a))), l.text = e.alt, l) : (null == e.nodeValue || void 0 === e.nodeValue ? l.text = "" : 0 == e.nodeValue.trim().length ? l.text = "" : (void 0 !== t && t.length > 0 && (e.nodeValue = e.nodeValue.replace(new RegExp(translate.util.regExp.pattern(t), "g"), translate.util.regExp.resultText(a))), l.text = e.nodeValue), l)
            }
        }, getNodeName: function (e) {
            return null == e || void 0 === e ? "" : null == e.nodeName || void 0 === e.nodeName ? "" : e.nodeName
        }, whileNodes: function (e, t) {
            if (null != t && void 0 !== t) {
                "object" == typeof t && "string" == typeof t.title && t.title.length > 0 && translate.addNodeToQueue(e, t, t.title, "title");
                var a = t.childNodes;
                if (a.length > 0) for (var n = 0; n < a.length; n++) translate.element.whileNodes(e, a[n]); else translate.element.findNode(e, t)
            }
    ...
    addNodeToQueue: function (e, t, a, n) {
        if (null != t && null != a && 0 != a.length && "#comment" != translate.element.getNodeName(t).toLowerCase()) {
            ...
            if (translate.util.findTag(a)) {
                ...
                if ("SCRIPT" == l || "STYLE" == l) return
            }
            var r = translate.language.get(a);
            for (var s in void 0 !== r[translate.to] && delete r[translate.to], r) {
                null != translate.nodeQueue[e].list[s] && void 0 !== translate.nodeQueue[e].list[s] || (translate.nodeQueue[e].list[s] = new Array);
                for (var o = 0; o < r[s].length; o++) if (void 0 !== r[s][o] && void 0 !== r[s][o].text) {
                    var u = r[s][o].text, i = r[s][o].beforeText, g = r[s][o].afterText, d = translate.util.hash(u);
                    if (null != translate.nodeQueue[e].list[s][d] && void 0 !== translate.nodeQueue[e].list[s][d] || (translate.nodeQueue[e].list[s][d] = new Array, translate.nodeQueue[e].list[s][d].nodes = new Array, translate.nodeQueue[e].list[s][d].original = u, translate.nodeQueue[e].list[s][d].translateText = translate.nomenclature.dispose(u), translate.nodeQueue[e].list[s][d].beforeText = i, translate.nodeQueue[e].list[s][d].afterText = g), void 0 !== t.isSameNode) for (var c = 0; c < translate.nodeQueue[e].list[s][d].nodes.length; c++) t.isSameNode(translate.nodeQueue[e].list[s][d].nodes[c].node);
                    var f = translate.nodeQueue[e].list[s][d].nodes.length;
                    translate.nodeQueue[e].list[s][d].nodes[f] = new Array, translate.nodeQueue[e].list[s][d].nodes[f].node = t, translate.nodeQueue[e].list[s][d].nodes[f].attribute = n
                }
            }
        }
    },
    language: {
        local: "", setLocal: function (e) {
            ...
            if (!(null != translate.language.local && translate.language.local.length > 2)) {
                ...
                if (null == e || void 0 === e || e.length < 1) translate.language.local = "chinese_simplified"; else {
                    e = e.replace(/\n|\t|\r/g, "");
                    for (var t = new Array, a = 0; a < e.length; a++) {
                        var n = e.charAt(a), l = translate.language.getCharLanguage(n);
                        "" == l && (l = "unidentification"), t.push(l)
                    }
                    var r = translate.util.arrayFindMaxNumber(t), s = r.indexOf("specialCharacter");
            ...
            for (var t = new Array, a = new Array, n = [], l = [], r = 0; r < e.length; r++) {
                var s = e.charAt(r), o = translate.language.getCharLanguage(s);
    ...
    request: {
        api: {
            ...
            this.send(e, t, a, "post", !0, {"content-type": "application/x-www-form-urlencoded"}, null)
        }, send: function (e, t, a, n, l, r, s) {
            var o = "";
            if (null != t) for (var u in t) o.length > 0 && (o += "&"), o = o + u + "=" + t[u];
            var i = null;
            try {
            ...
            if (i.open(n, e, l), null != r) for (var u in r) i.setRequestHeader(u, r[u]);
